...
const reducer = (state: State, action: any): State => {
  ...
  switch (action.type) {
    ...
    case 'session-state':
      if (
        state.session?.branch !== action.state.branch ||
        state.session?.commit_a != action.state.commit_a ||
        state.session?.commit_b != action.state.commit_b
      ) {
        return {
          ...state,
          session: action.state,
          diffPartial: undefined,
          stagedDiffPartial: undefined,
          stagedSummary: undefined,
          untrackedFiles: undefined,
          untrackedContents: new Map(),
        }
      }
    ...
    case 'diff':
      if (action.partial) {
        ...
        const kvs = action.diff?.files?.map((file: GitDiffFile) => [
          file.file_path,
          file,
        ])
        ...
        return {
          ...state,
          diffPartial: {
            from_commit: action.diff.from_commit,
            to_commit: action.diff.to_commit,
            files: files,
          },
        }
      }
    ...
    case 'untracked-content': {
      ...
      contents.set(action.path, {
        content: action.content,
        is_binary: action.is_binary,
      })
    ...
    <Box>
      <Text as="label" size="2">
        <Flex gap="2">
          ...
          />{' '}
          <Code color={checked ? 'pink' : 'gray'} size="1">
            --ignore-all-space
          </Code>
        </Flex>
      </Text>
            ...
            <Select.Group key={entry.path}>
              ...
              {entry.worktrees.map((wt) => (
                <Select.Item
                  ...
                  style={{
                    paddingLeft:
                      'calc(var(--select-item-indicator-width) + 12px)',
                  }}
                >
                  {wt.branch || displayPath(wt.path, root)}
                </Select.Item>
...
function AlgoSelect({ algo, onAlgoChange }: AlgoSelectProps) {
  return (
    <Flex direction="row" align="center" gap="1">
      <Code color={algo == 'myers' ? 'gray' : 'pink'} size="1">
        --diff-algorithm
      </Code>
      <Select.Root
        size="1"
      ...
      {state.session?.commit_a && (
        ...
        >
          ...
          <IconButton
            variant="soft"
            size="1"
            onClick={() => sendMsg({ type: 'reset-commit-a' })}
          >
            <Cross1Icon />
          </IconButton>
        </Flex>
      )}
      {(state.session?.commit_a || state.session?.commit_b) && (
        <IconButton
          size="1"
          variant="soft"
          onClick={() => sendMsg({ type: 'swap-commits' })}
        >
          <WidthIcon />
        </IconButton>
      )}
      {state.session?.commit_b && (
        ...
        >
          ...
          <IconButton
            variant="soft"
            size="1"
            onClick={() => sendMsg({ type: 'reset-commit-b' })}
          >
            <Cross1Icon />
          </IconButton>
        </Flex>
...
function ContextControl({ value, onInc, onDec, onReset }: ContextControlProps) {
  return (
    <Flex gap="1" direction="row" align="center">
      <IconButton color="gray" variant="soft" size="1" onClick={onInc}>
        <PlusCircledIcon />
      </IconButton>
      <IconButton color="gray" variant="soft" size="1" onClick={onDec}>
        <MinusCircledIcon />
      </IconButton>
      <IconButton color="gray" variant="soft" size="1" onClick={onReset}>
        <ResetIcon />
      </IconButton>
      <Code color={value === 3 ? 'gray' : 'pink'} size="1" wrap="nowrap">
        --unified={value}
      </Code>
    </Flex>
  )
...
function MaxCountControl({ value, onInc, onDec }: MaxCountControlProps) {
  return (
    <Flex gap="1" direction="row" align="center">
      ...
      <Code color={value === 25 ? 'gray' : 'pink'} size="1" wrap="nowrap">
        --max-count={value}
      </Code>
    </Flex>
  )
  ...
  return (
    <Flex direction="row" gap="1">
      <Button
        size="1"
        variant="soft"
        color={isSelectA ? 'cyan' : 'gray'}
        onClick={isSelectA ? unselectA : selectA}
      >
        A
      </Button>
      <Button
        size="1"
        variant="soft"
        color={isSelectB ? 'orange' : 'gray'}
        onClick={isSelectB ? unselectB : selectB}
      >
        B
      </Button>
    </Flex>
...
function Tag({ tag, session, sendMsg }: TagProps) {
  return (
    <Flex direction="row" align="center" gap="1" mb="1" mx="2">
      <ButtonAB
      ...
      <Text
        size="2"
        color={
          tag.name === session?.commit_a
            ? 'cyan'
            : tag.name === session?.commit_b
              ? 'orange'
              : undefined
        }
      >
    ...
    <Flex direction="column" m="2" key={commit.short_hash}>
      <Flex direction="row" gap="2">
        <ButtonAB
          ...
          selectA={() =>
            sendMsg({ type: 'set-commit-a', commit: commit.short_hash })
          }
          unselectA={() =>
            sendMsg({ type: 'reset-commit-a', commit: commit.short_hash })
          }
          selectB={() =>
            sendMsg({ type: 'set-commit-b', commit: commit.short_hash })
          }
          unselectB={() =>
            sendMsg({ type: 'reset-commit-b', commit: commit.short_hash })
          }
        />
        <Text
          color={
            commit.short_hash === session?.commit_a
              ? 'cyan'
              : commit.short_hash === session?.commit_b
                ? 'orange'
                : undefined
          }
        >
      ...
      <Box display="contents" className="commit-hover-wrapper">
        <Box>
          <Blockquote className="commit-summary" size="1" color="crimson">
            {commit.summary}
            {commit.body ? '(...)' : null}
          </Blockquote>
        </Box>
        <Box>
          <Blockquote className="commit-body" size="1" color="gold">
            {commit.body}
          </Blockquote>
        </Box>
      </Box>
...
function DiffSection({
  ...
  return (
    <Flex direction="column" mb="3">
      <Text as="div" m="1" mb="1" size="2" weight="bold" color={color}>
        {title}
      </Text>
      <Text
        as="div"
        ...
        size="1"
        color="gray"
      >{`${summary.total_files_changed} files, +${summary.total_additions} -${summary.total_deletions}`}</Text>
      <Accordion.Root
        type="multiple"
        value={session.open_paths}
        onValueChange={(paths: string[]) =>
          sendMsg({ type: 'set-open-paths', paths: paths })
        }
        ...
        {isWorkingTree ? (
          <>
            ...
            {untrackedFiles && untrackedFiles.length > 0 && (
              <Flex direction="column" mb="3">
                <Text as="div" m="1" mb="1" size="2" weight="bold" color="gray">
                  Untracked Files
                </Text>
                <Text
                  as="div"
                  ...
                  size="1"
                  color="gray"
                >{`${untrackedFiles.length} files`}</Text>
                <Accordion.Root
                  type="multiple"
                  value={openUntracked}
                  onValueChange={(paths: string[]) => {
                    const newlyOpened = paths.filter(
                      (p) => !openUntracked.includes(p),
                    )
          ...
          <>
            <Text
              ...
              mb="2"
              color="gray"
            >{`${summary.total_files_changed} files changed, ${summary.total_additions} insertions(+), ${summary.total_deletions} deletions(-)`}</Text>
            <Accordion.Root
              type="multiple"
              value={session.open_paths}
              onValueChange={(paths: string[]) =>
    ...
    <Toast.Root className="ToastRoot" open={open} onOpenChange={setOpen}>
      <Toast.Description asChild>
        <Callout.Root variant="soft" color="red">
          ...
          <Callout.Text>WS connection failed.</Callout.Text>
        </Callout.Root>
      </Toast.Description>
    </Toast.Root>
  )
...
function App() {
  ...
  const [commitSelectType, setCommitSelectType] =
    useState<CommitSelectType>('commits')
  ...
  const { sendMsg } = useWebSocket(
    ...
    (_: any) => setWsError(true),
    (_: any) => setWsError(true),
  )

  const CommitTypeSelect = (
  ...
  const Commits = (
    <Flex gridArea="commits" direction="column" justify="start" overflow="auto">
  ...
  const GitFetchButton = (
    <Button
      variant="soft"
      size="1"
      onClick={() => sendMsg({ type: 'git-fetch' })}
    >
      Fetch
      <UpdateIcon />
    </Button>
    ...
    >
      ...
      {state.session?.branch && (
        <BranchSelect
          ...
          onBranchChange={(branch) =>
            sendMsg({ type: 'branch-select', branch: branch })
          }
        />
      )}
      ...
      {state.session && (
        <AlgoSelect
          ...
          onAlgoChange={(algo) =>
            sendMsg({ type: 'set-diff-algo', algo: algo })
          }
        />
      )}
      {state.session && (
      ...
      {state.session?.git_flags && (
        <WhitespaceSwitch
          ...
          onCheckedChange={(checked: boolean) =>
            sendMsg({ type: 'ignore-all-space', value: checked })
          }
        />
      )}
        ...
        >
          ...
          {state.session && state.diffSummary && (
            <Diff
              session={state.session}
              sendMsg={sendMsg}
              diff={state.diffPartial}
              stagedDiff={state.stagedDiffPartial}
              summary={state.diffSummary}
              stagedSummary={state.stagedSummary}
